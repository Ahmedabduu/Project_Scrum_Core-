...
class TestProjectProjectBegin(base.BaseCase):
    ...
    def _check_start_end_dates_prepare(self, num_tasks, res):
        ...
        for name, start, end in self.project_init_dates:
            ...
            if res:
                ...
                self.assertEqual(
                    vals.get("date", False), date_end, "[%d] FAIL: date" % counter
                )
            else:
    ...
    def test_start_end_dates_prepare_one_task(self):
        self._check_start_end_dates_prepare(1, self.project_dates_res["one_task"])
    ...
    def _project_recalculate(self, num_tasks, res_project, res_tasks):
        ...
        for name, start, end in self.project_init_dates:
            ...
            # Set days (estimated_days and from_days to tasks)
            self.project_task_days_set(project, self.task_days[self.calculation_type])
            project.project_recalculate()
            # Check project dates
            dates = res_project[self.calculation_type][counter]
            self.assertEqual(
                project.date_start, dates[1], "[%d] FAIL: project date_start" % counter
            )
            self.assertEqual(
                project.date, dates[2], "[%d] FAIL: project date" % counter
            )
            # Check task dates
    ...
    def test_project_recalculate_no_task(self):
        self._project_recalculate(0, self.project_dates_res["no_tasks"], False)
